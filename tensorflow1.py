...
# similar to Theano, you need ot "feed" the variables values.
...
with tf.Session() as session:
    ...
    # va needs ot be of shape=(5, 1) not just shape=(5,)
...
# or more correctly, anythign that can be turned into a tf tensor
shape = (2, 2)
...
with tf.Session() as session:
    ...
    #eva() in tf is like get_value() in Theano
...
init = tf.initailize_all_variables()
with tf.Session() as session:
    session.run(init)
